...
/**
 * A client for sending metrics to a collectd daemon via a UNIX socket.
 * This class handles the connection, formatting, and sending of metric data
 * according to the collectd plain text protocol.
 */
...
public class CollectdClient {
    ...
    /**
     * Sends a metric to the collectd daemon.
     * It establishes a connection, formats the metric, sends it, and reads the response.
     *
    ...
    public enum CollectdType {
        ...
        GAUGE("gauge"),
        /** A counter is for values that only increase, like the number of requests. It will be reset on overflow. */
        COUNTER("counter"),
        /** A derive value is stored as a rate of change. It's like a counter but for non-monotonic values. */
        DERIVE("derive"),
